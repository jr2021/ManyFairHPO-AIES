...
from .mobohb_utils import TPESampler, GammaFunction, default_weights
...
class MOBOHB_CFG(base_config_generator):
    def __init__(self, configspace, max_budget, parameters, history_dir, run_id, init=True, min_points_in_model=None,
        ...
        if self.min_points_in_model < len(self.configspace.get_hyperparameters()) + 1:
            logger.warning('Invalid min_points_in_model value. Setting it to %i' % (
    ...
    def get_config(self, budget):
        ...
        logger.debug("history length", len(self.history))

        if len(self.history) > 0:
            budgets = [h['budget'] for h in self.history]
            budget_type, budget_counts = np.unique(budgets, return_counts=True)
            logger.debug("budget_type:{},budget_count:{}", budget_type, budget_counts)
        ...
        logger.debug(f"mobohb with init:{self.mobohb_with_init}")
        if self.mobohb_with_init:
            if np.random.rand() < self.random_fraction or self.init:
                sample = self.configspace.sample_configuration()
                info_dict['model_based_pick'] = False
                logger.warning("no model available, sampling from prior or init==True")
        else:
            if np.random.rand() < self.random_fraction or budget not in self.configs or not np.any(
                    budget_counts > self.min_points_in_model):
                sample = self.configspace.sample_configuration()
                info_dict['model_based_pick'] = False
                logger.warning("no model available, number_of_configuration for the budget < miminum"
                                  " number of points required or budget is not preset in config")

        if sample is None:
            try:
                # sample from largest budget
                logger.warning("USING TPE SAMPLER")
                idx_map = budget_counts > self.min_points_in_model
                largest_budget = max(budget_type)
    ...
    def new_result(self, job):
        ...
        logger.debug("Inside new result")
