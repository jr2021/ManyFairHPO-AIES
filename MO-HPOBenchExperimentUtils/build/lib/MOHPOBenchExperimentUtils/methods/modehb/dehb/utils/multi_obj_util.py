...
def get_eps_net_ranking(costs, index_list):
    ...
    """
    fronts, indices, front_index = nDS_index(np.array([np.array(x) for x in costs]), index_list)
    ...
    while n_selected < num_top:
        ...
        local_order = compute_eps_net(front, num_samples=(num_top - n_selected))
...
def minHV3D(costs, ref_point=[1, 1]):
    ...
    max_values = np.maximum(1.1 * max_values,  # case: value >
                            0.9 * max_values  # case: value < 0
                            )
    reference_point = max_values
    #to handle 0 values
    if np.all((np.array(reference_point) == 0.0)):
        reference_point = np.full(len(max_values), sys.float_info.epsilon)
    logger.debug("hv least constributor:{}", hv.least_contributor(reference_point))
