...
setuptools.setup(
    name='MO-HPOBenchExperimentUtils',
    author_email='philipp.mueller@stat.uni-muenchen.de',
    description='Tool for running MO Benchmarks',
    long_description=read_file('README.md'),
    long_description_content_type='text/markdown',
    license='Apache-2.0',
    version=read_file('MOHPOBenchExperimentUtils/__version__.py').split()[-1].strip('\''),
    packages=setuptools.find_packages(exclude=['*.tests', '*.tests.*',
                                               'tests.*', 'tests'],),
    package_data={'MOHPOBenchExperimentUtils': ['settings/*']},
