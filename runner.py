...
if __name__ == '__main__':
    ...
    parser.add_argument('--experiment_name', type=str, default='fairmohpo')
    parser.add_argument('--optimizer_name')
    parser.add_argument('--objective_name', type=str, default='f1_ddsp')
    parser.add_argument('--model_name', type=str, default='rf')
    parser.add_argument('--dataset_name', type=str, default='compas')
    parser.add_argument('--seed', type=int, default=0)
    parser.add_argument('--output_path', type=str, nargs='?', default='/work/dlclarge2/robertsj-fairmohpo',
                        help='name of folder where files will be dumped')
    args = parser.parse_args()

    # -------------------- CREATE THE OUTPUT DIRECTORY -----------------------------------------------------------------
    output_path = Path(args.output_path) / args.optimizer_name / args.objective_name / args.model_name / args.dataset_name / str(args.seed)
    ...
    # benchmark settings
    settings = all_settings[f'{args.experiment_name}_{args.model_name}']
    ...
    optimizer_settings['n_max_gen'] = get_n_max_gen(args.model_name, args.objective_name)
    optimizer_settings['pop_size'] = get_pop_size(args.model_name, args.objective_name)
    ...
    # runtime settings
    tae_limit = settings['optimization'].get('tae_limit', 1000) # use default
    wallclock_limit_in_s = 4 * 24 * 60 * 60
    estimated_cost_limit_in_s = 4 * 24 * 60 * 60
    is_surrogate = settings['optimization'].get('is_surrogate', False)

    # checkpointing
    def signal_handler(sig, frame):
        save_experiment(experiment, f'{args.experiment_name}.pickle')
    ...
    logger.info(f'POP_SIZE: {optimizer.algorithm.pop_size}')
    logger.info(f'TAE_LIMIT: {experiment.tae_limit}')
